:- module('oecd_test+https://read.oecd-ilibrary.org/taxation/model-tax-convention-on-income-and-on-capital-2017-full-version_g2g972ee-en',[]).
...
the knowledge base oecd_test includes:
    
% Fake member state laws
a person is liable to tax in a state under law123 of the state
if the person is in the state for more than 183 days.

a person is resident in a state
if the person is resident in the state under an article.
    
a person is resident in a state under Article4
if the person is liable to tax in the state.

a person is liable to tax in a state
if the person is liable to tax in the state under a law of the state.

the castle is of type immovable property under law1 of france.
    
%%%%
    
% OECD Convention on double taxation
