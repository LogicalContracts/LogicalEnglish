:- module('7_loan_agreement_with_problems+http://tests.com',[]).
...
the knowledge base 7_loan_agreement_with_problems includes:
...
the borrower cures on a date D2 the failure of an obligation  
	if the obligation is 
    	that the borrower pays an amount to the lender on a date D1
	and the borrower pays the amount to the lender on D2
    and the borrower notifies the lender on D2
    	that the borrower pays the amount to the lender on D2.
...
scenario one is:
    ...
    the borrower notifies the lender on 2016-06-02
    	that the borrower pays 525 to the lender on 2016-06-02.
...
query 4 is:
     which person pays which amount to which other person on which day.

query five is:
    which person one notifies which person two on which date that which eventuality. 
    
query six is:
    which one cures on which date the failure of which eventuality. 
...
?-  answer(one,with(4), le(Explanations), R).
*/