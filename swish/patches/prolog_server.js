...
(function(mod) {
  ...
  */
  ...
  CodeMirror.prototype.getTokenReferences = function(token) {
	...
	//console.log("getTokenReferences_token:"+JSON.stringify(token));
    
    if (!(token.type=="head_unreferenced"||token.type=="head"||
    	token.type=="goal_undefined"||token.type=="goal_local"||token.type=="file")){
			// worth a roundtrip to the server
			var myLocations=[];
			var state = this.state.prologHighlightServer;
			if (state.uuid)
				token["uuid"] = state.uuid;
			$.ajax({ url: state.url.xref,
				 data: token,
				 success: function(data) { myLocations = data; },
				 async:false
			});
	...
	var obj = {
	  ...
	  line:  token.line,
	  regex: new RegExp("\\b"+RegExp.escape(token.text), "g"), // hack to find predicte definition...
	  showAllMatches: true
	};
