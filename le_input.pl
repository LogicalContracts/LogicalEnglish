...
process_types_dict(Dictionary, Type_entries) :- 
    ...
    (   (member(dict([_|GoalElements], Types, _), Dictionary);
        ...
        process_types_or_names([], [Type], GoalElements, Types, TypeWords, _),
        concat_atom(TypeWords, '_', Word), Word\=''), Templates), 
    (Templates\=[] -> setof(is_type(Ty), member(Ty, Templates), Type_entries) ; Type_entries = []).

% process_types_or_names/6
process_types_or_names(V,[], _, _, [], V) :- nonvar(V), !.
:- if(exists_source(library(r/r_call))).
process_types_or_names(V, [Word|RestWords], Elements, Types, [the, chart|RestPrintWords], V1) :- 
    ...
    process_types_or_names(V, RestWords,  Elements, Types, RestPrintWords, V1).
:- endif.
process_types_or_names(V, [Word|RestWords], Elements, Types, PrintExpression, V1) :- 
...
process_types_or_names(V, [Word|RestWords], Elements, Types, PrintExpression, V2 ) :- 
    var(Word), %trace, 
    %print_message(informational, "Found variable: ~w for previous ~w"-[Word, V]),
    var_in(Word, V, PName), !, 
    process_name_var(PName, NameWorks), !, % Privilege for previous names
...
process_types_or_names(V, [Word|RestWords], Elements, Types, PrintExpression, V2 ) :- 
    var(Word), %trace, 
    matches_name(Word, Elements, Types, Name), !, % uses name from dict/3
    matches_type(Word, Elements, Types, Type), 
    process_name_var(Name, CNameWords),
    add_determiner(CNameWords, PrintName), 
...
process_types_or_names(V, [Word|RestWords], Elements, Types, [PrintWord|RestPrintWords], V1) :- 
    ...
    ((nonvar(Word), number(Word)) -> unparse_time(Word, PrintWord); PrintWord = Word), !, 
    process_types_or_names(V, RestWords,  Elements, Types, RestPrintWords, V1). 
process_types_or_names(V0, [Word|RestWords],  Elements, Types, Output, V2) :- % check it. Recursive call
    compound(Word), 
    le_answer:translate_goal_into_LE(V0, Word, PrintWord, V1), !, % cut the alternatives
    process_types_or_names(V1, RestWords,  Elements, Types, RestPrintWords, V2),
    append(PrintWord, RestPrintWords, Output). 
process_types_or_names(V, [Word|RestWords],  Elements, Types, [Word|RestPrintWords], V1) :-
    process_types_or_names(V, RestWords,  Elements, Types, RestPrintWords, V1).

% assumes that the name of the variable can have underscores like in amount_one or first_person
process_name_var(Name, CNameWords) :-
...
% expressions include dates and arithmetic expressions.
expression(X, InMap) --> dates(X, InMap).

% A temporary solution to parse 2022-02-06 as dates, not an expression with two minuses.
% A long term solution might be parsing according to the type of the variable, if it is a date, then parse it as a date.
expression(X, InMap, In, Out) :- not(dates(X, InMap, In, Out)), expr(X, InMap, In, Out), !.
...
addExp(X-Y, InMap) --> addExp(X, InMap), ['-'], mulExp(Y, InMap).
addExp(Out, InMap) --> mulExp(Out, InMap).
...
pad_number(Number, Width, Out) :-
     format(atom(Out), '~|~`0t~d~*+', [Number, Width]).
...
extract_variable(SW, InName, OutName, InType, OutType, [Word|RestOfWords], NextWords) :- % types are not part of the name
    ...
    extract_variable(SW, InName, NextName, InType, OutType, RestOfWords, NextWords),
    (NextName = [] -> OutName = [Word]; OutName = NextName), !.
extract_variable(SW, InName, [Word|OutName], InType, [Word|OutType], [Word|RestOfWords], NextWords) :- % everything else is part of the name (except numbers) (for instances) and the type (for templates)
    not(number(Word)),
    extract_variable(SW, InName, OutName, InType, OutType, RestOfWords, NextWords).
...
predef_meta_dict([nonvar, T1], [entity_X-thing], [T1, is, known]). % is it instantiated?
...
prolog_predef_dict([\=, T1, T2], [entity_X-thing, entity_Y-thing], [T1, is, different, from, T2]).
prolog_predef_dict([==, T1, T2], [entity_X-thing, entity_Y-thing], [T1, is, equivalent, to, T2]).
prolog_predef_dict([=, T1, T2], [entity_X-thing, entity_Y-thing], [T1, is, equal, to, T2]).
prolog_predef_dict([=<, T1, T2], [entity_X-thing, entity_Y-thing], [T1, is, less, or, equal, to, T2]).
prolog_predef_dict([<, T1, T2], [entity_X-thing, entity_Y-thing], [T1, is, less, than, T2]).
prolog_predef_dict([>=, T1, T2], [entity_X-thing, entity_Y-thing], [T1, is, greater, or, equal, to, T2]).
prolog_predef_dict([>, T1, T2], [entity_X-thing, entity_Y-thing], [T1, is, greater, than, T2]).
...
prolog_predef_dict([nonvar, T1], [entity_X-thing], [T1, is, known]). % is it instantiated?
prolog_predef_dict([=, T1, T2], [entity_X-thing, entity_Y-thing], [T1, is, T2]). % builtin Prolog assignment
% predefined entries:
%prolog_predef_dict([assert,Information], [info-clause], [this, information, Information, ' has', been, recorded]).
prolog_predef_dict([\=@=, T1, T2], [entity_X-thing, entity_Y-thing], [T1, \,=,@,=, T2]).
prolog_predef_dict([\==, T1, T2], [entity_X-thing, entity_Y-thing], [T1, \,=,=, T2]).
prolog_predef_dict([=\=, T1, T2], [entity_X-thing, entity_Y-thing], [T1, =,\,=, T2]).
prolog_predef_dict([=@=, T1, T2], [entity_X-thing, entity_Y-thing], [T1, =,@,=, T2]).
prolog_predef_dict([==, T1, T2], [entity_X-thing, entity_Y-thing], [T1, =,=, T2]).
prolog_predef_dict([=<, T1, T2], [entity_X-thing, entity_Y-thing], [T1, =,<, T2]).
prolog_predef_dict([>=, T1, T2], [entity_X-thing, entity_Y-thing], [T1, >,=, T2]).
prolog_predef_dict([is, T1, T2], [entity_X-thing, entity_Y-thing], [T1, =, T2]).
prolog_predef_dict([<, T1, T2], [entity_X-thing, entity_Y-thing], [T1, <, T2]).
prolog_predef_dict([>, T1, T2], [entity_X-thing, entity_Y-thing], [T1, >, T2]).


user_predef_dict([has_as_head_before, A, B, C], [list-list, symbol-term, rest_of_list-list], [A, has, B, as, head, before, C]).
user_predef_dict([same_date, T1, T2], [time_t1-time, time_t2-time], [T1, is, the, same, date, as, T2]). % see reasoner.pl before/2
user_predef_dict([is_1_day_after, A, B], [date_one-date, date_two-date],
                [A, is, '1', day, after, B]).
user_predef_dict([is_days_after, A, B, C], [date_one-date, number-number, date_two-date],
                  [A, is, B, days, after, C]).
user_predef_dict([immediately_before, T1, T2], [time_t1-time, time_t2-time], [T1, is, immediately, before, T2]). % see reasoner.pl before/2
user_predef_dict([is_not_before, T1, T2], [time_t1-time, time_t2-time], [T1, is, not, before, T2]). % see reasoner.pl before/2
user_predef_dict([isbeforeorequal, T1, T2], [time_t1-date, time_t2-date], [T1, is, before, or, equal, to, T2]). 
user_predef_dict([isafterorequal, T1, T2], [time_t1-date, time_t2-date], [T1, is, after, or, equal, to, T2]). 
user_predef_dict([isbefore, T1, T2], [time_t1-date, time_t2-date], [T1, is, before, T2]). 
user_predef_dict([isafter, T1, T2], [time_t1-date, time_t2-date], [T1, is, after, T2]).
...
var_in(Word, [(V,N-_)|_], N) :- Word == V, !. % using (V,N-_) to id vars and names
var_in(Word, [_|Rest], N) :- var_in(Word, Rest, N).
